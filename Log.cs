...
namespace ArdupilotMega
{
    public partial class Log : Form
    {
        ...
        List<Data> photodata  = new List<Data>();       // DWR mod 15/10/2013 temp storage var for photo locations
        ...
        private void Log_Load(object sender, EventArgs e)
        {
            ...
            MainV2.comPort.giveComport = true;      // 08/10/2013

            comPort = MainV2.comPort.BaseStream;

            comPort.DtrEnable = false;              // 08/10/2013
            comPort.RtsEnable = false;              // 08/10/2013
            ...
            var t11 = new System.Threading.Thread(delegate()
            {
                ...
                try
                {
                    comPort.Write("\n\n\n\nexit\r\nlogs\r\n"); // more in "connecting"  // 08/10/2013 was comPort.Write("\n\n\n\n"); // more in "connecting"
                }
                catch
                {
                }
                    ...
                    switch (status)
                    {
                        case serialstatus.Connecting:
                            ...
                            if (line.Contains("ENTER") || line.Contains("GROUND START") || line.Contains("reset to FLY") || line.Contains("interactive setup") || line.Contains("CLI") || line.Contains("Ardu"))
                            {
                                try
                                {
                                    System.Threading.Thread.Sleep(200); // 08/10/2013
                                    comPort.Write("\n\n\n\n");
                                }
                                catch { }

                                System.Threading.Thread.Sleep(500); // 08/10/2013
                        ...
                        case serialstatus.Done:
                            // 
                            // if (line.Contains("start") && line.Contains("end"))  // Commented out 08/10/2013
                            {
        ...
        private void processLine(string line)
        {
            ...
            try
            {
                ...
                if (items[0].Contains("CMD"))
                {
                    ...
                    {
                        if (int.Parse(items[3]) <= (int)MAVLink.MAV_CMD.LAST) // wps  // 08/10/2013 was items[2]
                        {
                ...
                //if (items[0].Contains("CAM"))       //       "CAM", "ILLeccC",   "GPSTime,Lat,Lng,Alt,Roll,Pitch,Yaw"  Need to add support ... DWR future mod
                //{
                    //double alt = double.Parse(items[6], new System.Globalization.CultureInfo("en-US"));
                    //roll = double.Parse(items[7], new System.Globalization.CultureInfo("en-US")) / -100;
                    //tilt = double.Parse(items[8], new System.Globalization.CultureInfo("en-US")) / -100;
                    //heading = double.Parse(items[9], new System.Globalization.CultureInfo("en-US")) / 100;
                //}
                ...
                //GPS, 1, 15691, 10, 0.00, -35.3629379, 149.1650850, -0.08, 585.41, 0.00, 126.89        // 08/10/2013

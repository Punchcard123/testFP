...
using System.Windows.Forms; // for messagebox for debugging purposes only
...
namespace ArdupilotMega
{
    public class CurrentState : ICloneable
    {
        ...
        // Photo
        [DisplayText("Photo Count")]
        public long photocnt { get; set; }
        [DisplayText("camera Photo pwm pulse value")]
        public float camPulseThreshold { get; set; }    // (float)(MainV2.comPort.MAV.param["CAM_SERVO_ON"]) * 0.9f;   //  > 90% cam on pwm value  07/01/2014
        public int camPulseRetnCh { get; set; }         // Input channel used for camera pulse   07/01/2014
        static float lastcamRetnChin = 0;               // 27/08/2013 DW Ralph we are hard coding ch5in as a photo event marker
        [DisplayText("Photo Latitude (dd)")]
        public float photolat { get; set; }         // 03/09/2013 DW Ralph  added (last) photo Approximate position suitable for placing position 
        [DisplayText("Photo Longitude (dd)")]
        public float photolng { get; set; }
        [DisplayText("Photo Altitude (dist)")]
        public float photoalt { get; set; }         // { get { return (_alt - altoffsethome) * multiplierdist; } set { _alt = value; } }
        public DateTime photolast = DateTime.MinValue;
            ...
            lock (locker)
            {
                ...
                if (bytearray != null)
                {
                    ...
                    if (bytearray != null)
                    {
                        ...
                        try
                        {
                            camPulseRetnCh = int.Parse(MainV2.comPort.MAV.param["CAM_RC_IN_SHUT"].ToString());   // will crash if AP not connected
                        }
                        catch (Exception) { }

                        switch (camPulseRetnCh) 
                        {
                            case 0: //"Disable":
                                camRetnChin = 0;
                                break;
                        ...
                        if (camRetnChin != lastcamRetnChin)
                        {
                            try
                            {
                                camPulseThreshold = (float)(MainV2.comPort.MAV.param["CAM_SERVO_ON"]) * 0.9f;   //  > 90% cam on pwm value
                            }
                            catch (Exception) { }

                            if (ch4in > camPulseThreshold)
                            {
                                photocnt++;
                                photolat = lat;
                                photolng = lng;
                                photoalt = alt;                                
                                // Ensure time hasnt gone backwards
                                if (photolast > datetime)
                                    photolast = datetime;
                            }
                            lastcamRetnChin = camRetnChin;
                        }

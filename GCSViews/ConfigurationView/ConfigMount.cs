...
namespace ArdupilotMega.GCSViews.ConfigurationView
{
    public partial class ConfigMount : UserControl, IActivate
    {
        ...
        public ConfigMount()
        {
            ...
            mavlinkComboBoxShutter.Items.AddRange(Enum.GetNames(typeof(ChannelCameraShutter)));
            mavlinkComboBoxDoor.Items.AddRange(Enum.GetNames(typeof(ChannelCameraDoor)));        /// 14/08/2014 DWR ///
            ...
            if (MainV2.comPort.MAV.cs.firmware == MainV2.Firmwares.ArduPlane)
            {
                ...
                //mavlinkComboBoxDoor.Items.AddRange(Enum.GetNames(typeof(Channelap)));        /// 13/08/2014 DWR ///
            }
            else
            {
                ...
                //mavlinkComboBoxDoor.Items.AddRange(Enum.GetNames(typeof(Channelac)));        /// 13/08/2014 DWR ///
            }
        }
                ...
                if (item.EndsWith("_FUNCTION"))
                {
                    switch (MainV2.comPort.MAV.param[item].ToString())
                    {
                        ...
                        case "8":
                            ...
                            break;
                        case "10":
                            mavlinkComboBoxShutter.Text = item.Replace("_FUNCTION", "");
                            break;
                        case "11":     // Use the "egg drop" param for Camera Door
                            mavlinkComboBoxDoor.Text = item.Replace("_FUNCTION", "");
                            break;
                        default:
                            break;
                    }
        ...
        {
            // shutter
            if (mavlinkComboBoxShutter.Text == "")
                return;

            if (mavlinkComboBoxShutter.Text != "Disable")
            {
                if (mavlinkComboBoxShutter.Text == ChannelCameraShutter.Relay.ToString())
                {
                    ensureDisabled(mavlinkComboBoxShutter, 10);  
                    MainV2.comPort.setParam("CAM_TRIGG_TYPE", 1);
                }
                else if (mavlinkComboBoxShutter.Text == ChannelCameraShutter.Transistor.ToString())
                {
                    ensureDisabled(mavlinkComboBoxShutter, 10);  
                ...
                {
                    ensureDisabled(mavlinkComboBoxShutter, 10);  
                    MainV2.comPort.setParam(mavlinkComboBoxShutter.Text + "_FUNCTION", 10);
                    // servo
            ...
            mavlinkNumericUpDownShutM.setup(800, 2200, 1, 1, mavlinkComboBoxShutter.Text + "_MIN", MainV2.comPort.MAV.param);
            mavlinkNumericUpDownShutMX.setup(800, 2200, 1, 1, mavlinkComboBoxShutter.Text + "_MAX", MainV2.comPort.MAV.param);
            ...
            mavlinkNumericUpDownTimeInterval.setup(35, 250, 1, 5, "CAM_TIME_INTERVA", MainV2.comPort.MAV.param);  // in 1/100th sec
            //mavlinkNumericUpDownDistInterval.setup(20, 250, 1, 1, "CAM_DIST_INTERVA", MainV2.comPort.MAV.param);     // in metres  
            mavlinkNumericUpDownDistInterval.setup(20, 250, 1, 1, "CAM_TRIGG_DIST", MainV2.comPort.MAV.param);     // in metres
            CMB_inputch_shutter.setup(typeof(Channelinput), "CAM_RC_IN_SHUT", MainV2.comPort.MAV.param);    // 29/10/2013
        }
        ...
        void updateCameraDoor()
        {
            // Camera Door
            if (mavlinkComboBoxDoor.Text == "")
                return;

            if (mavlinkComboBoxDoor.Text != "Disable")
            {
                MainV2.comPort.setParam(mavlinkComboBoxDoor.Text + "_FUNCTION", 11);
            }
            else
            {
                ensureDisabled(mavlinkComboBoxDoor, 11);
            }

            mavlinkNumericUpDownDSM.setup( 800, 2200, 1, 1, mavlinkComboBoxDoor.Text + "_MIN", MainV2.comPort.MAV.param);
            mavlinkNumericUpDownDSMX.setup(800, 2200, 1, 1, mavlinkComboBoxDoor.Text + "_MAX", MainV2.comPort.MAV.param);
            mavlinkNumericUpDownDoorClose.setup(800, 2200, 1, 1, "CAM_DOOR_CLOSE", MainV2.comPort.MAV.param);
            mavlinkNumericUpDownDoorOpen.setup( 800, 2200, 1, 1, "CAM_DOOR_OPEN", MainV2.comPort.MAV.param);
            mavlinkCheckBoxDR.setup(-1, 1, mavlinkComboBoxDoor.Text + "_REV", MainV2.comPort.MAV.param);
            CMB_inputch_door.setup(typeof(Channelinput), "CAM_RC_IN_DOOR", MainV2.comPort.MAV.param);
        }
        ...
        private void mavlinkComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                ensureDisabled(cmb, 10, mavlinkComboBoxShutter.Text);
                ensureDisabled(cmb, 11, mavlinkComboBoxDoor.Text);
